...
const OrderEdit = () => {
  ...
  useEffect(() => {
    orderService.getById(id).then(res => setFormData(res.data)).catch(() => navigate('/admin/order'));
  }, [id, navigate]);

  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true);
    orderService.update(id, formData).then(() => { setLoading(false); navigate('/admin/order'); }).catch(err => { setLoading(false); alert('Lỗi: ' + (err.response?.data || err.message)); });
  };
      ...
      <div className="form-container card-panel">
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-actions">
            <Link to="/admin/order" className="btn-secondary">Hủy</Link>
