...
import { Trash2, Plus, Edit, Shield, Search, Filter, ShoppingBag, Users } from 'lucide-react';
...
const CustomerList = () => {
  ...
  const handleDelete = (id) => {
    if (window.confirm('Bạn có chắc chắn muốn xoá khách hàng này? Toàn bộ dữ liệu liên quan sẽ bị xoá!')) {
      customerService.delete(id).then(() => fetchData()).catch(err => alert('Lỗi: ' + err.message));
    }
  };
    ...
    <div className="admin-page">
      <div className="page-header">
        <div className="flex-row">
          <h2><Users /> Quản Lý Khách Hàng</h2>
          <span className="badge bg-blue-100 text-blue-700">{data.length} Tổng số</span>
        </div>
        <Link to="/admin/customer/create" className="btn-primary"><Plus size={18} /> Thêm Khách Hàng</Link>
      </div>
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      />
                      <div>
                        <div className="font-bold text-dark">{item.name || item.username} {item.roles?.includes('ADMIN') && <span style={{fontSize: '10px', background: '#fef2f2', color: '#ef4444', padding: '2px 6px', borderRadius: '4px', marginLeft: '6px'}}>ADMIN</span>}</div>
                  ...
                  <td style={{textAlign: 'right'}}>
                    <div className="action-btns" style={{justifyContent: 'flex-end'}}>
                      <Link to={'/admin/customer/edit/' + item.id} className="btn-icon text-blue" title="Chỉnh sửa chi tiết">
                        <Edit size={18}/>
                      </Link>
