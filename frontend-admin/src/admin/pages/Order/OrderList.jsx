...
const OrderList = () => {
  ...
  const fetchData = () => {
    setLoading(true);
    orderService.getAll().then(res => { setData(res.data); setLoading(false); }).catch(err => { console.error(err); setLoading(false); });
  };
  ...
  return (
    <div className="admin-page">
      <div className="page-header">
        <h2><FileText /> Quản Lý Order</h2>
        <Link to="/admin/order/create" className="btn-primary"><Plus size={18} /> Thêm Mới</Link>
      </div>
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    <div className="action-btns">
                      <Link to={'/admin/order/edit/' + item.id} className="btn-icon text-blue"><Edit size={18}/></Link>
