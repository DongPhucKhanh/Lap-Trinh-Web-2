...
const ProductEdit = () => {
  ...
  // existing galleries that user hasn't deleted
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let finalImage = formData.image; // default old image
      if (imageFile) {
            ...
            <div style={{display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '10px'}}>
              ...
              {galleryPreviews.map((src, idx) => (
                <div key={`new-${idx}`} style={{position: 'relative', width: '100px', height: '100px'}}>
                  <img src={src} alt="Gallery Preview" style={{width: '100%', height: '100%', objectFit: 'cover', borderRadius: '8px', border: '2px dashed #3b82f6'}} />
