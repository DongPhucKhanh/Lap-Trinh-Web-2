...
const UserEdit = () => {
  ...
  useEffect(() => {
    userService.getById(id).then(res => setFormData(res.data)).catch(() => navigate('/admin/user'));
  }, [id, navigate]);

  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true);
    userService.update(id, formData).then(() => { setLoading(false); navigate('/admin/user'); }).catch(err => { setLoading(false); alert('Lỗi: ' + (err.response?.data || err.message)); });
  };
